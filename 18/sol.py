...
print(all_sect_sums([1, 2, 3]))

def search(pepts, pmasses, real_masses):
    if not (pmasses <= real_masses):
        return
    print(max(pmasses), pepts, pmasses)
    if max(pmasses) == max(real_masses):
        answer.append("-".join(map(str, pepts)))
        return
    for m in set(pept_mass.values()):
        npepts = copy(pepts)
        npmasses = copy(pmasses)
        npepts.append(m)
        for mi in pmasses:
            npmasses.add(mi + m)
        search(npepts, all_sect_sums(npepts), real_masses)
